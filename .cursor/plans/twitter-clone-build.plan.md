...
- **Decision:** Profile pages use `src/app/(main)/[username]/page.tsx`. Static routes (`home`, `settings`) are defined explicitly in the same `(main)` group.
...
- **Impact on plan:** The upload API route and `profilePicture` field on User are designed to store a URL regardless of provider. The choice only affects the upload implementation in `src/lib/upload.ts`.
...
```bash
npm install prisma @prisma/client bcryptjs jose disposable-email-domains
...
mkdir -p src/app/\(verify\)/verify-email
mkdir -p src/app/\(main\)/home
mkdir -p src/app/\(main\)/settings
mkdir -p src/app/\(main\)/\[username\]
...
- `src/lib/jwt.ts` -- JWT sign, verify, and decode helpers using `jose`
- `src/lib/password.ts` -- bcrypt hash/compare wrappers
- `src/lib/auth.ts` -- `getCurrentUser()` helper, `requireUser()` helper
- `src/lib/email-validation.ts` -- disposable email filter + email format validation
- `src/lib/constants.ts` -- reserved usernames list, validation rules
- `src/middleware.ts` -- JWT verification + route protection (see AD-3)
- `src/app/(auth)/signup/page.tsx` -- signup form
- `src/app/(auth)/login/page.tsx` -- login form
- `src/app/(verify)/verify-email/page.tsx` -- "check your email" pending page
- Server Actions for signup, login, logout, resend verification

### JWT Helper (`src/lib/jwt.ts`)
...
### Disposable Email Filter (`src/lib/email-validation.ts`)
...
### Middleware (`src/middleware.ts`) -- JWT verification on Edge (AD-3)
...
### `getCurrentUser()` helper (`src/lib/auth.ts`)
...
- Server Action to create tweet
- Server-side validation: non-empty, max 280 chars, user authenticated via `getCurrentUser()`
...
- **Username change validation:**
  ...
  - Not already taken (check with `db.user.findUnique`)
...
**Implementation:** Create `src/lib/rate-limit.ts` with a reusable `rateLimit(key, limit, windowMs)` function. Call it at the top of each Server Action.
...
Content-Security-Policy: default-src 'self'; img-src 'self' [image-host-tbd]; style-src 'self' 'unsafe-inline';
Strict-Transport-Security: max-age=31536000; includeSubDomains
```

Note: The `img-src` CSP directive will be updated once the profile picture storage provider is chosen (see AD-7).
